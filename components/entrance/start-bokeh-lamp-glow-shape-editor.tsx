...
import {
  clearStartBokehLampGlowOverrides,
  saveStartBokehLampGlowOverrides,
} from "@/lib/entrance/start-bokeh-lamp-glow-dev-overrides";
import { formatStartBokehLampGlowsForCopy } from "@/lib/entrance/lamp-glow-format";
import {
...
type StartBokehLampGlowShapeEditorProps = {
...
/** 読み込み直後ボケ玉 — 光の大きさ・形・強さをライブ調整 */
export function StartBokehLampGlowShapeEditor({
  glows,
  selectedId,
  onSelect,
  onPatch,
  ...
  groupTabs,
}: StartBokehLampGlowShapeEditorProps) {
  const [panelHidden, setPanelHidden] = useState(false);
  const selected = glows.find((glow) => glow.id === selectedId) ?? glows[0];
  const fileGlow = START_BOKEH_LAMP_GLOWS.find((glow) => glow.id === selectedId);
  ...
  return (
    <div className="pointer-events-auto absolute inset-x-0 bottom-[4%] z-[70] flex flex-col items-center gap-2 px-3">
      ...
      <p className="font-mono text-[9px] text-stone-500">
        読み込み直後ボケ玉 — 確定値は{" "}
        <code className="text-stone-400">lib/entrance/start-bokeh-lamp-glows.ts</code>
      </p>
      ...
      <LampGlowCopyActions
        ...
        onCopySave={saveStartBokehLampGlowOverrides}
        onClearOverrides={clearStartBokehLampGlowOverrides}
        formatForCopy={formatStartBokehLampGlowsForCopy}
      />
    </div>
  );
}
