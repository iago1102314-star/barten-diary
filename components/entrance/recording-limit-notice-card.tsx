...
import { RECORDING_LIMIT_NOTICE_TUNING as T } from "@/lib/entrance/recording-limit-notice-tuning";
import { EASE_SOFT } from "@/lib/entrance/motion-presets";
import { AnimatePresence, motion, useReducedMotion } from "motion/react";
import { useEffect, useState, type CSSProperties } from "react";

type RecordingLimitNoticeCardProps = {
  onDismiss: () => void;
};

/** 録音3分上限 — 暗転中の案内（チュートリアル同型・黒レイヤーなし・自動退場） */
export function RecordingLimitNoticeCard({
  onDismiss,
}: RecordingLimitNoticeCardProps) {
  ...
  return (
    <AnimatePresence onExitComplete={onDismiss}>
      {open ? (
        <motion.div
          key="recording-limit-notice-card"
          className={styles.stage}
          style={stageStyle}
          role="status"
          aria-live="polite"
          aria-labelledby="recording-limit-notice-title"
          initial={
            instant
              ? false
              : {
        ...
        >
          ...
          >
            <article
              ...
              style={cardStyle}
            >
              <h2 id="recording-limit-notice-title" className={styles.title}>
                {T.title}
              </h2>
              <div
