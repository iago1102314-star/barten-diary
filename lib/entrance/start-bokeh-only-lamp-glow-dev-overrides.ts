...
const STORAGE_KEY = "barten-start-bokeh-only-lamp-glow-overrides";
...
function isValidOverrides(value: unknown): value is StartBokehLampGlowConfig[] {
  if (!Array.isArray(value) || value.length !== START_BOKEH_ONLY_LAMP_GLOWS.length) {
    return false;
  }

  const expectedIds = new Set(START_BOKEH_ONLY_LAMP_GLOWS.map((glow) => glow.id));
  return value.every(
    (item) =>
      item &&
      typeof item === "object" &&
...
export function loadStartBokehOnlyLampGlowOverrides(): StartBokehLampGlowConfig[] | null {
...
export function saveStartBokehOnlyLampGlowOverrides(
  glows: StartBokehLampGlowConfig[],
): void {
...
export function clearStartBokehOnlyLampGlowOverrides(): void {
  if (typeof window === "undefined") return;
  window.localStorage.removeItem(STORAGE_KEY);
}

export function mergeStartBokehOnlyLampGlowsForShapeEdit(): StartBokehLampGlowConfig[] {
  const draft = loadStartBokehOnlyLampGlowOverrides();

  return START_BOKEH_ONLY_LAMP_GLOWS.map((fileGlow) => {
    ...
    return {
      ...
      colorRgb: draftGlow.colorRgb ?? fileGlow.colorRgb,
    };
  });
}

/** 光調整パッチ — offsetX/Y は常に TS ファイルの値を維持 */
export function applyStartBokehOnlyLampGlowShapePatch(
  glows: StartBokehLampGlowConfig[],
  id: string,
  patch: Partial<StartBokehLampGlowShapeFields>,
): StartBokehLampGlowConfig[] {
  const fileById = new Map(
    START_BOKEH_ONLY_LAMP_GLOWS.map((glow) => [glow.id, glow]),
  );
